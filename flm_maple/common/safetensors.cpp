...
void SafeTensors::_open_file() {
    ...
    if (std::filesystem::is_directory(path)) {
        if (std::filesystem::exists(path + "/model.q4nx")) {
            path += "/model.q4nx";
        } else if (std::filesystem::exists(path + "/model.safetensors")) {
...
size_t SafeTensors::_get_data_size(tensor_metadata tensor_meta) {
    return tensor_meta.byte_size;
}
